...
class PassportPhotoEditor:
    def __init__(self, root):
        self.root = root
        self.root.title("Passport Photo Editor v1.0.0")
    ...
    def setup_ui(self):
        ...
        left_panel = tk.Frame(content_frame, bg="white", relief=tk.SUNKEN, bd=2)
        left_panel.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 10))
        
        tk.Label(left_panel, text="Image Preview", bg="white", font=("Arial", 10, "bold")).pack(pady=5)
        self.image_label = tk.Label(left_panel, bg="white", text="No image loaded")
        self.image_label.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # Right panel - Controls
        right_panel = tk.Frame(content_frame, bg="white", relief=tk.SUNKEN, bd=2)
        right_panel.pack(side=tk.RIGHT, fill=tk.Y, padx=(10, 0), pady=0)
        right_panel.config(width=280)
        right_panel.pack_propagate(False)
        
        tk.Label(right_panel, text="Adjustments", bg="white", font=("Arial", 11, "bold")).pack(pady=10)
        
        # Brightness slider
        self.brightness_frame = tk.Frame(right_panel, bg="white")
        self.brightness_frame.pack(fill=tk.X, padx=15, pady=8)
        tk.Label(self.brightness_frame, text="Brightness", bg="white", font=("Arial", 9)).pack(anchor=tk.W)
        self.brightness_scale = tk.Scale(self.brightness_frame, from_=0.5, to=2.0, resolution=0.05, 
        ...
        self.brightness_label = tk.Label(self.brightness_frame, text=f"{self.brightness_val:.2f}x", 
        ...
        tk.Label(self.contrast_frame, text="Contrast", bg="white", font=("Arial", 9)).pack(anchor=tk.W)
        self.contrast_scale = tk.Scale(self.contrast_frame, from_=0.5, to=2.0, resolution=0.05, 
                                       orient=tk.HORIZONTAL, command=self.update_preview, bg="white")
        self.contrast_scale.set(self.contrast_val)
        self.contrast_scale.pack(fill=tk.X, pady=5)
        self.contrast_label = tk.Label(self.contrast_frame, text=f"{self.contrast_val:.2f}x", 
                                       bg="white", fg="#666", font=("Arial", 9))
        ...
        tk.Label(self.color_frame, text="Saturation", bg="white", font=("Arial", 9)).pack(anchor=tk.W)
        self.color_scale = tk.Scale(self.color_frame, from_=0.5, to=2.0, resolution=0.05, 
                                    orient=tk.HORIZONTAL, command=self.update_preview, bg="white")
        self.color_scale.set(self.color_val)
        self.color_scale.pack(fill=tk.X, pady=5)
        self.color_label = tk.Label(self.color_frame, text=f"{self.color_val:.2f}x", 
                                    bg="white", fg="#666", font=("Arial", 9))
        ...
        tk.Label(self.sharpness_frame, text="Sharpness", bg="white", font=("Arial", 9)).pack(anchor=tk.W)
        self.sharpness_scale = tk.Scale(self.sharpness_frame, from_=0.5, to=2.0, resolution=0.05, 
                                        orient=tk.HORIZONTAL, command=self.update_preview, bg="white")
        self.sharpness_scale.set(self.sharpness_val)
        self.sharpness_scale.pack(fill=tk.X, pady=5)
        self.sharpness_label = tk.Label(self.sharpness_frame, text=f"{self.sharpness_val:.2f}x", 
                                        bg="white", fg="#666", font=("Arial", 9))
        self.sharpness_label.pack(anchor=tk.E)
        
        # Preset buttons
        tk.Label(right_panel, text="Presets", bg="white", font=("Arial", 11, "bold")).pack(pady=(15, 10))
    ...
    def open_image(self):
        ...
        if file_path:
            ...
            self.update_preview()
            self.root.title(f"Passport Photo Editor - {os.path.basename(file_path)}")
